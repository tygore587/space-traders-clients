...
export const Faction = ({factionData}:IFactionData) =>
{
    ...
    return (
        <div className="
            ...
            h-fit w-fit" 
        >
            <p className="text-orange-600 font-bold">{faction.name} - {faction.symbol}</p>
            <p className="text-orange-600 font-bold text-xs">{faction.headquarters}</p>
            <p className="italic">{faction.description}</p>
            <hr className="border-orange-600 mx-[0.4em] my-[1em]"/>
            <ul className="list-disc list-inside mt-[0.4em] pl-[0.2em] flex flex-col gap-2 marker:text-orange-600">
